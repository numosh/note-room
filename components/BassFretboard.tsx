...
import { BassVoicing, Note } from '../types';
import { BASS_TUNING, getNoteOnFretboard } from '../utils/musicTheory';

interface BassFretboardProps {
  voicing: BassVoicing | null;
  chordName: string;
  scaleNotes: Note[];
  chordNotes: Note[];
  rootNote: Note;
...
const BassFretboard: React.FC<BassFretboardProps> = ({ voicing, chordName, scaleNotes, chordNotes, rootNote }) => {
  ...
  const renderNoteDot = (stringIndex: number, fret: number) => {
    const note = getNoteOnFretboard(BASS_TUNING[stringIndex], fret);
  ...
  return (
    <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 overflow-x-auto">
       <h3 className="text-xl font-bold text-gray-200 mb-4">5-String Bass Fretboard <span className="text-base font-normal text-gray-400">- {voicing ? `Arpeggio for ${chordName}` : 'Scale View'}</span></h3>

      <div className="relative" style={{ minWidth: '700px', height: '150px' }}>
        {/* Frets */}
        ...
        {[3, 5, 7, 9, 12].map(fret => (
           <div key={`marker-${fret}`} className="absolute top-1/2 -translate-y-1/2 flex justify-center items-center z-0" style={{ left: `calc(40px + ${fret * (100/(NUM_FRETS + 1.5))}% - ${(100/(NUM_FRETS + 1.5))/2}% )` }}>
              <div className={`w-4 h-4 rounded-full ${fret === 12 ? 'bg-gray-500/80' : 'bg-gray-600'}`} />
               {fret === 12 && <div className="absolute -top-8 w-4 h-4 rounded-full bg-gray-500/80" />}
               {fret === 12 && <div className="absolute top-8 w-4 h-4 rounded-full bg-gray-500/80" />}
           </div>
        ))}
        
        {/* Scale & Voicing Notes */}
        {BASS_TUNING.map((_, stringIndex) => 
